...
static void set_config(bds::BDSSDKMessage &cfg_params) {
    ...
    const std::string app_id = "16223658";
    const std::string app_key = "qj0T1r6rORw6SjCeEfzxGXUu";
    const std::string app_secret = "jjGwYIpau45PER1dgk3EQopVftGit71d";

    const std::string product_id = "15373"; // 普通话搜索模型(没有逗号）)：1536，普通话搜索模型+语义理解 15361, 普通话输入法模型（有逗号） 1537
    ...
    DIR* dir = opendir("/home/voice-demo/sample/asrDemo2/pcm");
    	...
    	while((p = readdir(dir)) != NULL)
    	{
        	if(p->d_name[0] != '.')
        	{
            		...
            		//cout<<filename<<endl;
            		//std::string str=filename.substr(7);
			string pref="..//pcm//";
		        string filename=pref+name.c_str();
			std::string str=filename.substr(7);
			output=str.substr(0,str.rfind(".pcm"))+".txt";
		        ...
		        //FILE *fp=NULL;
		        //fp=fopen("./radio/1.txt","a+");
				//strcpy(x,filename[i].c_str());
		        //fputs(x,fp);
		        //fputs("我是分隔\n\n\n\n",fp);
		        //fclose(fp);
				//新增
				//oldName="/home/voice-demo/sample/asrDemo2/run/radio/1.txt";
		        //newName="/home/voice-demo/sample/asrDemo2/run/radio/";
		        //newName=newName.c_str()+filename[i];
